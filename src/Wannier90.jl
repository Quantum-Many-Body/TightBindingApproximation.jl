...
"""
    readlattice(path::AbstractString, prefix::AbstractString; name::Symbol=Symbol(prefix), projection::Bool=true) -> Lattice{3, Float64, 3}
...
"""
function readlattice(path::AbstractString, prefix::AbstractString; name::Symbol=Symbol(prefix), projection::Bool=true)
    content = open(io->read(io, String), joinpath(path, prefix*".win"), "r")
...
"""
    readcenters(path::AbstractString, prefix::AbstractString) -> Matrix{Float64}

Read the centers of Wannier functions obtained by Wannier90.
"""
function readcenters(path::AbstractString, prefix::AbstractString)
    content = open(io->read(io, String), joinpath(path, prefix*"_centres.xyz"), "r")
...
"""
    readhamiltonian(path::AbstractString, prefix::AbstractString) -> OperatorSum{W90Hoppings, NTuple{3, Int}}

Read the hamiltonian from the "_hr.dat" file generated by Wannier90.
"""
function readhamiltonian(path::AbstractString, prefix::AbstractString)
    lines = open(readlines, joinpath(path, prefix*"_hr.dat"), "r")
...
"""
    W90(path::AbstractString, prefix::AbstractString; name::Symbol=Symbol(prefix), projection::Bool=true)

Construct a quantum lattice system based on the files obtained from Wannier90.
"""
function W90(path::AbstractString, prefix::AbstractString; name::Symbol=Symbol(prefix), projection::Bool=true)
...
"""
    readbands(path::AbstractString, prefix::AbstractString) -> Tuple{Vector{Float64}, Matrix{Float64}}

Read the band structure from Wannier90 files by setting "bands_plot = .true.".
"""
function readbands(path::AbstractString, prefix::AbstractString)
    nk = open(joinpath(path, prefix*"_band.dat"), "r") do f
        count = 0
        while !eof(f)
